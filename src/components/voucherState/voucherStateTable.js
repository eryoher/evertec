...
class VoucherStateTable extends Component {
    ...
    handleGetCant = (e) => {
        ...
        this.props.salesAffectedState({ ComprobAvencer: comprobante, OpcionMuestra });
    }

    handleChangeSelect = (value) => {
        const { ComprobAvencer } = this.state;
        this.setState({ OpcionMuestra: value });
        this.props.salesAffectedState({ ComprobAvencer, OpcionMuestra: value });
    }
...
const mapStateToProps = ({ vouchertype, salesAffected }) => {
    ...
    const { productsState, cantValidate, subCalculations, salesconfirm } = salesAffected;
    return { voucherType, productsState, cantValidate, subCalculations, salesconfirm };
};
